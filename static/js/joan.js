  ...
  $(document).ready(() => {
    ...
    defineMaterialByUser.on("click", (e) => {
      ...
      $(e.target)
        ...
        const part = partsArray.find(part => part.name === $(e.target).parents("td").prev().text());
        part.definedBy = $(e.target).text();
    });
    ...
    const nodeRowSample = $("tr.node").eq(0).clone(true, true);
    ...
    function showLoadsSummary(sortedData) {
      ...
      sortedData.forEach((dataItem) => {
        ...
        const fastener = fastArray.find(
          (fastener) =>
            (fastener.firstNode.colRow[0] === node.colRow[0] && fastener.firstNode.colRow[1] === node.colRow[1])
            || (fastener.secondNode.colRow[0] === node.colRow[0] && fastener.secondNode.colRow[1] === node.colRow[1])
        );
    ...
    function onFilesSelect(e) {
      ...
      for (var i = 0; i < files.length; i++) {
        ...
        fileReader.onload = function(e) {
          ...
          fileNameInput[0].dispatchEvent(event);
        }
    ...
    saveModelLink.click(function() {
      ...
      this.href = `data:application/txt;charset=utf-8, ${encodeURIComponent(data)}`;
      const fileName = fileNameInput.val();
      if (fileName) {
        this.download = fileName + (fileName.endsWith('.txt') ? '' : '.txt');
      }
    })
    ...
    function createModelfromInputFile() {
      ...
      partTableBody.html('');
      partTableBody.append(fillRow(partRowSample, [[1, 'Part a'], [3, ''], [4, '']]));
      partsArray.forEach(part => {
        ...
        if (existPartRow) {
          ...
          $(existPartRow).find(".col2 button").text(part.definedBy);
        } else {
          ...
          $(newPartRow).find(".col2 button").text(part.definedBy);
      ...
      nodeTableBody.html('');
      nodeTableBody.append(fillRow(nodeRowSample, [[1, ''], [2, ''], [3, ''], [4, ''], [5, '']]));
      ...
      plateTableBody.html('');
      plateTableBody.append(fillRow(plateRowSample, [[1, ''], [2, ''], [3, '']]));
      ...
      fastTableBody.html('');
      fastTableBody.append(fillRow(fastRowSample, [[1, ''], [3, ''], [4, ''], [5, ''], [6, ''], [7, ''], [8, ''], [9, 1], [10, 1]]));
      ...
      reactTableBody.html('');
      reactTableBody.append(fillRow(reactRowSample, [[1, ''],[2, '']]));
      ...
      loadTableBody.html('');
      loadTableBody.append(fillRow(loadRowSample, [[1, ''],[2, '']]));
